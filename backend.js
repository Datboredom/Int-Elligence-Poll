...
const STRINGS = {
    ...
    clientIdMissing: missingValue('client ID', 'EXT_CLIENT_ID'),
    ownerIdMissing: missingValue('owner ID', 'EXT_OWNER_ID'),
    answerBroadcast: 'Sending answers to %s',
    messageSendError: 'Error sending message to channel %s: %s',
    pubsubResponse: 'Message to c:%s returned %s',
...
function getAnswers(req) {
    ...
    return currentColor;
    */
    return attemptAnswerBroadcast(channelId);
}

function answerQueryHandler(req) {
    //Verify all requests.
    const payload = verifyAndDecode(req.headers.authorization);

    //Get the color for the channel from the payload and return it.
    const { channel_id: channelId, opaque_user_id: opaqueUserId } = payload;
    verboseLog(STRINGS, opaqueUserId);
    return payload;
}

function attemptAnswerBroadcast(channelId) {
    //Check the cool-down to determine if it's okay to send now.
    const now = Date.now();
    const cooldown = channelCooldowns[channelId];
    if (!cooldown || cooldown.time < now) {
        //It is.
        return 'okay to send now';
        channelCooldowns[channelId] = { time: now + channelCooldownMs };
    } else if (!cooldown.trigger) {
        //It isn't; schedule a delayed broadcast if we haven't already done so.
        cooldown.trigger = setTimeout(sendAnswerBroadcast, now - cooldown.time, channelId);
    }
}

function sendAnswerBroadcast(channelId) {
    //Set the HTTP headers required by the Twitch API.
    const headers = {
        'Client-ID': clientId,
        'Content-Type': 'application/json',
    ...
    //Send the broadcast request to the Twitch API.
    verboseLog(STRINGS.answerBroadcast, channelId);
    request(
        `https://api.twitch.tv/extensions/message/${channelId}`,
        {
            method: 'POST',
